...
def set_stations_and_categories(df):
    ...
    ------
        ...
        set_stations: list of set of stations (states)
        set_cats: list of set of categories
    """
    noticias = df['noticia']
    stations, cats = [], []
    for idx_noticia in range(len(noticias)):
        stations_by_text, cats_by_text = [], []
        # Removing punctuation 
        text = remove_punctuation(noticias.values[idx_noticia])
        ...
        for word in words:
            if word in SIGLAS_ESTADOS:   
                stations_by_text.append(word.lower())
        ...
        for idx_estado in range(len(ESTADOS)):    
            if ESTADOS[idx_estado] in text: 
                stations_by_text.append(SIGLAS_ESTADOS[idx_estado].lower())
                
        # seek for CAPITAIS name
        for idx_capitais in range(len(CAPITAIS)):    
            if CAPITAIS[idx_capitais] in text: 
                stations_by_text.append(SIGLAS_ESTADOS[idx_capitais].lower())
        ...
        stations.append(stations_by_text)
    # Removing replicated items
    set_stations = [set(station) for station in stations]
    set_cats = [set(cat) for cat in cats]
    
    df = df.assign(estacoes = set_stations)
    df = df.assign(categorias = set_cats)
    return df, set_stations, set_cats

if __name__ == '__main__':
    # columns = links, noticia, titulos, categorias, estacoes   
    df = pd.read_csv('/home/diego/Dropbox/Python/crawling/resultados-uol2.csv', index_col=0)
     
    df, set_stations, set_cats = set_stations_and_categories(df)
    df.to_csv('resultados-categorias-tag.csv', encoding='utf-8')
                  
    name_file = 'categorias-tag.txt' 
    print('Writing table file')
    f = open(name_file, 'w')
    for idx_cats in range(len(set_cats)):
        f.write('\n' + str(idx_cats) + ' - estacoes: ' + str(set_stations[idx_cats]))
